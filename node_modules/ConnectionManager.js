...
class ConnectionManager {
    constructor(app) {
        ...
        app.get("/countries/", this.getCountryList.bind(this))
            .get("/competitions/", this.getAllCompetitions.bind(this))
            .get("/competition/:idcompetition/results/:matchday", this.getCompetitionResults.bind(this)) .get("/competition/:idcompetition/rankings", this.getCompetitionRankings.bind(this))
            .get("/competition/:idcompetition/rankings/:matchday", this.getCompetitionRankings.bind(this))
            .get("/competition/:idcompetition", this.getCompetitionInfo.bind(this))
            .get("/teams/:teamid/roster", this.getTeamRoster.bind(this))
            .get("/teams/:teamid", this.getTeamInfo.bind(this))
            .get("/player/:teamid/:playerid", this.getPlayerInfo.bind(this)) // Récupérer l'effectif puis prendre les infos de l'id du joueur
    ...
    getCompetitionResults(req, res) {
        let url = this.utils.generateRequest({request_type: "results", competition_id: req.params.idcompetition, matchday: req.params.matchday});


        return this.getData(url).then((data) => {
          for(let fixture of data.fixtures) {
            if(fixture.result.goalsHomeTeam > fixture.result.goalsAwayTeam) {
              fixture.final_res = "homeWin"
            } else if(fixture.result.goalsHomeTeam < fixture.result.goalsAwayTeam) {
              fixture.final_res = "awayWin";
            } else {
              fixture.final_res = "draw";
            }
          }
    ...
    getCompetitionRankings(req, res) {
        let url = this.utils.generateRequest({request_type: "ranking", competition_id: req.params.idcompetition, "matchday": req.params.matchday});
    ...
    getCompetitionsByCountry(req, res) {
        let competitions = [],
            competition_ids = this.utils.getCompetitionsByCountryShort(req.params.countryid),
            url = this.utils.generateRequest({"request_type": "competitions"}),
            country_class = this.utils.getCountryCaption(req.params.countryid);



        return this.getData(url).then((results) => {
                for(let competition of results) {
                    if(competition_ids.indexOf(competition.id) !== -1) {
                        competition.country_id = country_class;

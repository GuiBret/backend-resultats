class Utils {
    constructor() {
        this.info = [
            {
                country_short: "en",
                name:"Angleterre",
                id: "england", 
                competitions : {
                    "PL": {
                        id:445
                    },
                    "ELC": {
                        id:446
                    },
                    "EL1": {
                        id:447
                    },
                    "EL2": {
                        id:448
                    }
                }
                
            },
            {
                country_short: "nl",
                name: "Pays-Bas",
                id: "netherlands",
                competitions: {
                    "DED": {
                        id:449
                    }
                } 
            },
            {
                country_short:"fr",
                name: "France", 
                id:"france",
                competitions: {
                    "FL1": {
                        id: 450
                    },
                    "FL2": {
                        id:451
                    }
                }             
            },
            {
                country_short: "de",
                name: "Allemagne",
                id:"germany",
                competitions: {
                    "BL1" : {
                        id: 452
                    
                    },
                    "BL2": {
                        id: 453
                    },
                    "DFB": {
                        id: 458
                    }
                }
                
            },
            {
                country_short: "es",
                name:"Espagne",
                id:"spain",
                competitions: {
                    
                    "PPL": {
                        id:457
                    }
                }
                    
                
                
            },
            {
                country_short: "pt",
                name:"Portugal",
                id:"portugal",
                competitions: {
                    "PD": {
                        id:455
                    }
                }
            },
            {
                country_short: "it",
                name: "Italie",
                id:"italy",
                competitions: {
                    "SA": {
                        id:456
                    },
                    "SB": {
                        id:459
                    }
                }
            },
            {
                country_short: "eu",
                name: "Europe",
                id:"europe",
                competitions: {
                    "CL": {
                        id: 464        
                    }
                }
            },
            {
                country_short: "au",
                name: "Australie",
                id:"australia", 
                competitions : {
                    "AAL": {
                        id:466
                    }
                }
            },
            {
                country_short: "br",
                name:"Brésil",
                id: "brazil", 
                competitions : {
                    "BSA": {
                        id: 444
                    }
                }
            }
        ]
    }
    
    generateRequest(params) {
        let url = "http://api.football-data.org/v1";
        
        switch(params.request_type) {
            case "competitions": 
                url += "/competitions";
                (typeof params.season !== "undefined") ? url += `?season=${params.season}` : ""; 
            break;
            case "results":
                url += `/competitions/${params.competition_id}/fixtures`;
                (typeof params.matchday !== "undefined") ? url += `?matchday=${params.matchday}` : ""; 
            break;
            case "ranking":
                url += `/competitions/${params.competition_id}/leagueTable`;
                (typeof params.matchday !== "undefined") ? url += `?matchday=${params.matchday}` : ""; 
            break;
            case "teams_in_championship":
                url += `/competitions/${params.competition_id}/teams`;
            break;
            case "team_roster": 
                url += `/teams/${params.team_id}/players`;
            break;
            case "team_info": 
                url += `/teams/${params.team_id}`;
            break;     
            case "competition_info": 
                url += `/competitions/${params.competition_id}/`
            break;
        }
        
        return url;
    }

    getCountryByCompetitionId(competition_id) {
        let result = [];
        
        for(let country of this.info) {
            if(Object.keys(country.competitions).indexOf(competition_id) >= 0) { // If the competition's ID is in the "competitions" JSON structure 
                return country;
            }
        }
        return -1; 
    }
    
    getCountryById(country_short) {
        for(let country of this.info) {
            if(country.country_short === country_short) {
                return country;
            }
        }
        
        return -1;
    }
    
    getCountryList() {
        let countryList = [];
        for(let country of this.info) {
            countryList.push({"id": country.country_short, "name": country.name, "country_text": country.id});
        }
        return countryList;
    }
    
    getCompetitionsByCountryShort(country_short) {
        let country = this.getCountryById(country_short),
            result = [];
        if(country === -1) {
            return -1;
        } 
        
        Object.keys(country.competitions).forEach((key) => {
            result.push(country.competitions[key].id);
        });    
        
        return result;
    }
    
    getQualificationPlaces(id) {
        switch(id) {
            case "445": // Premier League
                return {
                    "c1": [2,3,4],
                    "c3": [5],
                    "promotion": [],
                    "playoffsup": [],
                    "playoffsdown": [],
                    "relegation": [18,19,20]
                    
                }
            break;
            case "446": // Championship
                return {
                    "c1": [],
                    "c3": [],
                    "promotion": [2],
                    "playoffsup": [3,4,5,6],
                    "playoffsdown": [],
                    "relegation": [22,23,24]
                }
            break;
            case "448": // League Two
                return {
                    "c1": [],
                    "c3": [],
                    "promotion": [2,3],
                    "playoffsup": [4,5,6,7],
                    "playoffsdown": [],
                    "relegation": [23,24]
                }
            break;
            case "449": // Eredivisie
                return {
                    "c1": [2],
                    "c3": [3,4],
                    "promotion": [],
                    "playoffsup": [5,6,7,8],
                    "playoffsdown": [16,17],
                    "relegation": [18]
                }
            break;
            case "450": // Ligue 1
                return {
                    "c1": [2, 3],
                    "c3": [4],
                    "promotion": [],
                    "playoffsup": [],
                    "playoffsdown": [18],
                    "relegation": [19, 20]
                }
            break;
            case "451": // Ligue 2
                return {
                    "c1": [],
                    "c3": [],
                    "promotion": [2],
                    "playoffsup": [3,4,5],
                    "playoffsdown": [18],
                    "relegation": [19,20]
                }
            break;
            case "452": // 1. Bundesliga
                return {
                    "c1": [2,3,4],
                    "c3": [5,6],
                    "promotion": [],
                    "playoffsup": [],
                    "playoffsdown": [16],
                    "relegation": [17,18]
                }
            break;
            case "453": // 2. Bundesliga
                return {
                    "c1": [],
                    "c3": [],
                    "promotion": [2],
                    "playoffsup": [3],
                    "playoffsdown": [16],
                    "relegation": [17,18]
                }
            case "456": // Serie A
                return {
                    "c1": [2,3],
                    "c3": [4,5],
                    "promotion": [],
                    "playoffsup": [],
                    "playoffsdown": [],
                    "relegation": [18,19,20]
                    
                }
            case "457":
                return {
                    "c1": [2],
                    "c3": [3,4],
                    "promotion": [],
                    "playoffsup": [],
                    "playoffsdown": [],
                    "relegation": [17,18]
                }
            break;
            default:
                return {
                    "c1": [],
                    "c3": [],
                    "promotion": [2],
                    "playoffsup": [3,4,5,6],
                    "playoffsdown": [],
                    "relegation": [22,23,24]
                }
            break;
        }
    }
    
    getCountryClass(competition_id) { // Inserts an attribute "id" in the response for each competition so that the frontend is colored according to the country of origin
        let selected_country = this.getCountryByCompetitionId(competition_id);
        
        return selected_country.id;
    }
    
    getCountryCaption(country_short) {
        for(let country of this.info) {
            if(country_short === country.country_short)
                return country.id;
        }
        
        return "";
    }
}

module.exports = Utils;
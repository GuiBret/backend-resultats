class Utils {
    ...
    getCountryList() {
        let countryList = [];
        for(let country of this.competitionsByCountry) {
            countryList.push({"id": country.country_id, "name": country.name, "country_text": country.id});
        }
        return countryList;
    }
    
    getCompetitionsByCountryId(country_id) {
        let country = this.getCountryById(country_id),
            result = [];
        if(country === -1) {
            return -1;
        } 
    ...
    getCountryCaption(country_short) {
        for(let country of this.competitionsByCountry) {
            if(country_short === country.country_id)
                return country.id;
        }
